...
int main() {
    ...
    std::cout << "CAUTION: DO NOT RESUME TRAINING UNLESS OTHER TRAINING NOT RUNNING\n";
    ...
    cmd << "mkdir -p LOGDIR && sbatch batch_workflow_manager_resume.sh config.json";

    int ret = std::system(cmd.str().c_str());
    if (ret != 0) {
        std::cerr << "Error: batch_workflow_manager_resume.sh failed with code " << ret << "\n";
    }
