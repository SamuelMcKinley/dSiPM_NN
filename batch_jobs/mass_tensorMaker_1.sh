#!/bin/bash
#SBATCH -J "mass_tensorMaker_1.sh"
#SBATCH -N 1
...
while squeue -u "$USER" | grep -q "batch_wo"; do
  ...
  if [ ! -f "start_tensorMaker_1.txt" ]; then
    sleep 1
  else
    echo "Found start_tensorMaker_1.txt"

    # Wait until the communication file is ready and populated
    while true; do
      if [ -s "start_tensorMaker_1.txt" ]; then
        sleep 2
        . start_tensorMaker_1.txt
        echo "✅ Loaded variables: particle=${particle}, energy=${energy}, group=${group}, SPAD_Size=${SPAD_Size}"
        break
      else
        echo "⏳ Waiting for start_tensorMaker_1.txt to be ready..."
        sleep 2
      fi
    done

    # Now that variables are sourced, wait for the ROOT file
    root_file="mc_Simulation_run0_1_Test_1evt_${particle}_${energy}_${energy}.root"
    ...
    python3 -u ${home_dir}/tensorMaker.py     mc_Simulation_run0_1_Test_1evt_${particle}_${energy}_${energy}.root     ${energy}     tensor_${group}_1_${particle}_${energy}     ${SPAD_Size}

    # Delete simulation file
    rm -rf mc_Simulation_run0_1_Test_1evt_${particle}_${energy}_${energy}.root

    mv tensor_${group}_1_${particle}_${energy}/npy/* tensfold/tensor_1_${particle}_${energy}_${group}_${SPAD_Size}.npy
    rm -rf tensor_${group}_1_${particle}_${energy}
