...
echo "Searching for start_Simulations_0.txt"
while squeue -u "$USER" | grep -q "batch_wo"; do
  ...
  if [ ! -f "start_Simulations_0.txt" ]; then
    sleep 1
  else
    echo "Found start_Simulations_0.txt"

    # Copy over variables: ${particle}, ${energy}
    . start_Simulations_0.txt
    ...
    echo "/random/setSeeds $seed1 $seed2" > random_0.mac
    cat ${sim_dir}/paramBatch03_single.mac >> random_0.mac

    # Run GEANT4 simulation from directory parallel to dSiPM_NN
    singularity exec --cleanenv --bind /lustre:/lustre /lustre/work/yofeng/SimulationEnv/alma9forgeant4_sbox/       bash -c "source /workspace/geant4-v11.2.2-install/bin/geant4.sh &&         ${sim_dir}/exampleB4b -b random_0.mac \
        -jobName Simulation -runNumber 0 -runSeq 0 \
